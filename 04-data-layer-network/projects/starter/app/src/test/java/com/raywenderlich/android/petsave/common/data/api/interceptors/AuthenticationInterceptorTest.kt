...
class AuthenticationInterceptorTest {
    ...
    fun authenticationInterceptor_validToken() {
        ...
        okHttpClient.newCall(
            Request.Builder().url(mockWebServer.url(ApiConstants.ANIMALS_ENDPOINT))
                .build()
        ).execute()
    ...
    fun authenticatorInterceptor_expiredToken() {
        ...
        verify(preferences).getToken()
        verify(preferences).putToken(validToken)
        verify(preferences).getTokenExpirationTime()
        verify(preferences).putTokenExpirationTime(anyLong())
        verify(preferences).putTokenType(ApiParameters.TOKEN_TYPE.trim())
